...
import { useRegisterForm, useGetPlatforms, useGetPaymentMethods, useCreateCalendar, useCreateCalendarEvent } from "@/apis/hooks";
...
export default function RegisterForm({
    ...
    const { mutateAsync: createCalendar, isPending: isLoadingCreateCalendar } = useCreateCalendar();
    const { mutateAsync: createCalendarEvent, isPending: isLoadingCreateCalendarEvent } = useCreateCalendarEvent();
    ...
    const handleRegisterForm = async () => {
        ...
        try {
            ...
            if (res?.subscription_id && user?.google_calendar_id) {
    ...
    useEffect(() => {
        if (user?.user_id && !user?.google_calendar_id) {
            createCalendar({
                user_id: user?.user_id ?? "",
            });
        }
    }, [user?.user_id, user?.google_calendar_id]);
            ...
            <div className="flex flex-col w-full max-w-lg bg-black border border-gray-600 rounded-xl p-8 shadow-xl overflow-y-auto max-h-[90vh] hidde-scrollbar">
                {
                    user?.google_calendar_id ? (
                                ...
                                </div>
                                {
                                    !isLoadingPaymentMethods && !isLoadingPlatforms && (
                                        <button
                                            ...
                                            type="button"
                                        >
                                            {isLoadingRegisterForm ? "Guardando..." : isLoadingCreateCalendarEvent ? "Creando evento en Google Calendar..." : "Guardar suscripción"}
                                        </button>
                                    ) 
                                }
                            </div>
                        ...
                        <div className="w-full flex flex-col items-center justify-center gap-6">
                            ...
                            {
                                !isLoadingCreateCalendar && user?.google_calendar_id && (
                                    <div className="flex flex-col items-center justify-center">
                                        <span className="font-geist text-base text-gray-300 text-center">Conexión realizada correctamente.</span>
                                    </div>
                                )
                            }
                        </div>
